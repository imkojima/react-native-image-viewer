    ...
    ImageViewer.prototype.getContent = function () {
        ...
        var ImageElements = this.props.imageUrls.map(function (image, index) {
          ...
          {children}
            ...
            var failImageWidth = _this.props.failImageSource.width || screenWidth;
            var failImageHeight = _this.props.failImageSource.height || screenHeight;
            // 如果宽大于屏幕宽度,整体缩放到宽度是屏幕宽度
            if (failImageWidth > screenWidth) {
                var widthPixel = screenWidth / failImageWidth;
                failImageWidth *= widthPixel;
                failImageHeight *= widthPixel;
            }
            // 如果此时高度还大于屏幕高度,整体缩放到高度是屏幕高度
            if (failImageHeight > screenHeight) {
                var HeightPixel = screenHeight / failImageHeight;
                failImageWidth *= HeightPixel;
                failImageHeight *= HeightPixel;
            }
            switch (imageInfo.status) {
                case 'loading':
                    return (<Wrapper key={index} style={_this.styles.modalContainer} imageWidth={_this.props.failImageSource ? failImageWidth : screenWidth} imageHeight={_this.props.failImageSource ? failImageHeight : screenHeight}>
              {_this.props.failImageSource &&
                        _this.props.renderImage({
                            source: _this.props.failImageSource.url,
                            style: {
